...
| MOBI / AZW / AZW3 | mobi crate → 原始字节判码 → 图片内联 → 按 `<mbp:pagebreak/>`、标题或长度切章节 | `<mbp:pagebreak/>`、`<h1>/<h2>/<h3>` 或长度 fallback | DRM 加密不支持；部分 KF8-only/AZW3 当前无法可靠解析，会停止显示乱码并提示换用无 DRM 版本 |
...
阅读页顶部「书签」按钮会把当前 `book_id + spine_index + scroll_y` 保存到 `bookmarks` 表，并附带目录标签和一小段正文预览。首页顶部「书签」按钮打开右侧抽屉，列表可滚动、可搜索，点击可直接进入对应章节 / PDF 页和滚动位置。

### 使用帮助

书架顶部和阅读页顶部都有「使用帮助」按钮。点击后打开右侧抽屉，纯前端内置内容，不读写外部文档、不新增依赖、不影响现有数据。帮助内容分为 6 个快速卡片：

- 导入与书架：选择书库、刷新、推荐、全文搜。
- 阅读操作：目录、翻页/滚动、阅读设置、书签。
- 标注与笔记：选中文字、选择颜色、查看标注、导出。
- AI 问答：问当前章节/整本书/全书库、保存 AI 笔记。
- 实体提取：提取人物地名、点击下划线查看解释。
- AI 配乐：按章节情绪推荐音乐。

底部固定给出首次使用建议流程：导入书库 → 打开一本书 → 选中文字做标注 → 打开问 AI → 提取实体。

### 书籍评分

每本书可打 1–5 星个人评分：

- 书架卡片底部显示 5 个星号，点击星号立即写入 `books.user_rating`。
- 阅读页顶部也显示同一评分入口，读书时不用返回书架。
- 再次点击当前星级会清空评分。
- 评分是本地个人数据，和豆瓣缓存评分分开；豆瓣评分仍只作为 hover 元数据展示。
...
3. 「扫描书库」按钮 → `ipc.scanLibrary()` 仍保留，用于强制全量扫描：
   ...
   - `looks_like_a_book()` 启发式过滤：
     ...
     - 落在系统目录（node_modules / .git / \_\_MACOSX 等）跳过
...
书架顶部集中放常用入口：推荐、书签、笔记、音乐、统计、AI 设置、使用帮助、重新扫描、AI 整理、换目录、从 Calibre 导入。帮助面板和 AI 设置一样使用右侧抽屉交互。

### 拖入书架

在桌面版中，可以把 EPUB / PDF / TXT / DOCX / MOBI / AZW / AZW3 文件直接拖到书架窗口。应用会把文件复制到当前书库根目录，随后触发扫描并刷新书架。拖入文件夹时，会递归复制其中支持的电子书文件；不支持的文件会跳过。
...
阅读页顶部「实体」按钮会对当前章节调用 `ai_extract_entities`，提取人名、地名及一句简介。结果显示在右侧实体面板，并在正文中给对应实体加下划线；点击下划线可以查看简介。该功能复用全局 AI 设置，不会自动索引整本书。
...
目前包含累计阅读时长（`books.read_time_ms`）和个人书籍评分（`books.user_rating`）。阅读时长在书架卡片右下显示；评分在书架卡片和阅读页顶部均可编辑。
...
- **Kindle 加密 / KF8-only**：加密 AZW/MOBI 不能解析正文；部分 KF8-only/AZW3 目前会被识别为不可可靠解析，应用会给出明确提示而不是显示整页乱码。
