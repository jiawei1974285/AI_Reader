...
fn inline_mobi_images(body: &str, images: &[String]) -> String {
    ...
    let out = IMG_TAG_RE.replace_all(body, |caps: &regex::Captures| -> String {
        ...
        // 3. Fallback — unknown tag syntax; map sequentially. This is the
        //    "Calibre wrote img00001.jpg-style src" case. Image records in
        //    a MOBI almost always appear in document order, so sequential
        //    assignment is reliable when the explicit refs are missing.
        if seq_counter < images.len() {
            if first_unmatched.is_none() {
                first_unmatched = Some(attrs.chars().take(120).collect());
            }
            let data_uri = &images[seq_counter];
            seq_counter += 1;
            by_sequential += 1;
            return format!(r#"<img src="{}" />"#, data_uri);
        }
        // 4. Ran out of records — drop the tag (the CSS would hide it
        //    anyway, so dropping is cleaner than leaving broken markup).
        if first_unmatched.is_none() {
            first_unmatched = Some(attrs.chars().take(120).collect());
        }
        dropped += 1;
    ...
    if total > 0 || !images.is_empty() {
        eprintln!(
            "[mobi/aireader] inline_mobi_images: {tags} <img> tags, {recs} image records \
             | matched: recindex={ri} kindle={ki} sequential={seq} dropped={dr} \
...
fn resolve_image(n: usize, images: &[String]) -> String {
    if n == 0 || n > images.len() {
        return String::new();
    }
    format!(r#"<img src="{}" />"#, images[n - 1])
}
